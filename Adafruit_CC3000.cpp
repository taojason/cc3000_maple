...
uint8_t g_csPin, g_irqPin, g_vbatPin;
...
#define CHECK_SUCCESS(func,Notify,errorCode)  {if ((func) != CC3000_SUCCESS) { if (CC3KPrinter != 0) CC3KPrinter->println(Notify); return errorCode;}}
...
bool Adafruit_CC3000::begin(uint8_t patchReq)
{
  ...
  // printf("check wlan init done!\n\r");
  DEBUGPRINT_F("start\n\r");

  wlan_start(patchReq);
  // printf("check wlan start done!\n\r");
...
bool Adafruit_CC3000::connectSecure(const char *ssid, const char *key, int32_t secMode)
{
  ...
  if ( (secMode < 0) || (secMode > 3)) {
    if (CC3KPrinter != 0) CC3KPrinter->println("Security mode must be between 0 and 3");
    return false;
  }
...
void Adafruit_CC3000::connectToAP(const char *ssid, const char *key, uint8_t secmode) {
  ...
  do {
    ...
    if ((secmode == 0) || (strlen(key) == 0)) {
      ...
      if (! connectOpen(ssid)) {
        if (CC3KPrinter != 0) CC3KPrinter->println("Failed!");
        continue;
      }
    } else {
    ...
    if (CC3KPrinter != 0) CC3KPrinter->print("Waiting to connect...");
    ...
    if (timer <= 0) {
      if (CC3KPrinter != 0) CC3KPrinter->println("Timed out!");
    }
...
Adafruit_CC3000_Client Adafruit_CC3000::connectTCP(uint32_t destIP, uint16_t destPort)
{
  ...
  if (-1 == tcp_socket)
  {
    if (CC3KPrinter != 0) CC3KPrinter->println("Failed to open socket");
  ...
  {
    if (CC3KPrinter != 0) CC3KPrinter->println("Connection error");
